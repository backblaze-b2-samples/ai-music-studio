...
def test_generation_http_only_in_music_provider():
    """Mirror of `test_boto3_only_in_repo` for generation-API HTTP clients.

    The `MusicProvider` abstraction in `app/repo/music_provider.py` is the
    only place in the codebase allowed to call a real generation API.
    Service / runtime code goes through `get_provider()` and must never
    reach `requests` or `httpx` to talk to a generation backend itself.

    `httpx` is allowed in `tests/` (FastAPI's test client uses it), so
    the scan only covers `app/`. `requests` is on the deny-list because
    the project doesn't ship it as a runtime dep — the rule is "the
    moment someone adds it, it has to be inside music_provider.py".
    """
    violations = []
    allowed_file = APP_ROOT / "repo" / "music_provider.py"
    for pyfile in _get_python_files(APP_ROOT):
        if pyfile == allowed_file:
            continue
        for imp in _get_imports(pyfile):
            if imp == "requests" or imp.startswith("requests."):
                rel = pyfile.relative_to(APP_ROOT.parent)
                violations.append(f"{rel}: requests imported outside music_provider.py")
            if imp == "httpx" or imp.startswith("httpx."):
                rel = pyfile.relative_to(APP_ROOT.parent)
                violations.append(f"{rel}: httpx imported outside music_provider.py")
    assert violations == [], (
        "MusicProvider boundary violations:\n" + "\n".join(violations)
    )
